...
from finance.models import FeeMasterModel, OutstandingFeeModel
...
class TimeTableCreateView(LoginRequiredMixin, PermissionRequiredMixin, SuccessMessageMixin, CreateView):
    model = ClassTimeTableModel
    permission_required = 'academic.add_subjectsmodel'
    form_class = ClassTimeTableForm
    success_message = 'Time Table Added Successfully'
    template_name = 'academic/timetable/create.html'

    def get_success_url(self):
        url = reverse('timetable_index')
        params = {"student_class": self.object.student_class.id, "class_section": self.object.class_section.id}
        params_url = "{}?{}".format(url, urlencode(params))
        return params_url
    ...
    def get_context_data(self, **kwargs):
        ...
        if school_setting.separate_school_section:
            context['class_list'] = ClassesModel.objects.filter(type=self.request.user.profile.type).order_by('name')
            context['academic_info'] = AcademicSettingModel.objects.filter(type=self.request.user.profile.type).first()
        else:
            context['class_list'] = ClassesModel.objects.all().order_by('name')
            context['academic_info'] = AcademicSettingModel.objects.first()
        return context

    def get_form_kwargs(self):
        kwargs = super(TimeTableCreateView, self).get_form_kwargs()
        school_setting = SchoolGeneralInfoModel.objects.first()
        if school_setting.separate_school_section:
            kwargs.update({'type': self.request.user.profile.type})
        kwargs.update({'type': self.request.user.profile.type})
...
class TimeTableListView(LoginRequiredMixin, PermissionRequiredMixin, ListView):
    ...
    permission_required = 'academic.view_subjectsmodel'
    fields = '__all__'
    template_name = 'academic/timetable/index.html'
    context_object_name = "timetable_list"

    def get_queryset(self):
        student_class = self.request.GET.get('student_class', None)
        class_section = self.request.GET.get('class_section', None)

        return ClassTimeTableModel.objects.filter(student_class=student_class, class_section=class_section,
    ...
    def get_context_data(self, **kwargs):
        ...
        class_section = self.request.GET.get('class_section', None)
        if student_class:
            context['student_class'] = ClassesModel.objects.get(pk=student_class)
        if class_section:
            context['class_section'] = ClassSectionModel.objects.get(pk=class_section)
        school_setting = SchoolGeneralInfoModel.objects.first()
        if school_setting.separate_school_section:
            context['class_list'] = ClassesModel.objects.filter(type=self.request.user.profile.type).order_by('name')
            context['academic_info'] = AcademicSettingModel.objects.filter(type=self.request.user.profile.type).first()
...
class TimeTableUpdateView(LoginRequiredMixin, PermissionRequiredMixin, SuccessMessageMixin, UpdateView):
    model = ClassTimeTableModel
    permission_required = 'academic.change_subjectsmodel'
    form_class = ClassTimeTableEditForm
    success_message = 'Time Table Updated Successfully'
    template_name = 'academic/timetable/edit.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        if school_setting.separate_school_section:
            ...
            context['class_list'] = ClassesModel.objects.all().order_by('name')
            context['academic_info'] = AcademicSettingModel.objects.first()
        context['timetable'] = self.object
        return context

    def get_form_kwargs(self):
        kwargs = super(TimeTableUpdateView, self).get_form_kwargs()
        school_setting = SchoolGeneralInfoModel.objects.first()
        if school_setting.separate_school_section:
            kwargs.update({'type': self.request.user.profile.type})
        kwargs.update({'type': self.request.user.profile.type})
...
class TimeTableDeleteView(LoginRequiredMixin, PermissionRequiredMixin, SuccessMessageMixin, DeleteView):
    model = ClassTimeTableModel
    permission_required = 'academic.delete_subjectsmodel'
    success_message = 'Time Table Deleted Successfully'
    fields = '__all__'
    template_name = 'academic/timetable/delete.html'
    context_object_name = "timetable"

    def get_success_url(self):
        timetable = self.get_object()
        url = reverse('timetable_index')
        params = {"student_class": timetable.student_class.id, "class_section": timetable.class_section.id}
        params_url = "{}?{}".format(url, urlencode(params))
        return params_url
...
def confirm_admin_password(request):
    password = request.GET.get('password').lower()
