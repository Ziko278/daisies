...
def result_create_view(request):
    ...
    if school_setting.separate_school_section:
        class_list = ClassesModel.objects.filter(type=request.user.profile.type, result_type='score').order_by('name')
        text_class_list = ClassesModel.objects.filter(type=request.user.profile.type, result_type='text').order_by('name')
        ...
        text_class_list = ClassesModel.objects.filter(result_type='text').order_by('name')
...
def result_upload_view(request):
    if request.method == 'POST':
        ...
        for num in range(len(student_list)):
            ...
            midterm_percent = round((midterm_total/midterm_max * 100), 1)
            ...
            for result_grade in grade_list:
                ...
                if (midterm_percent >= result_grade.min_mark) and midterm_percent < result_grade.max_mark:
        ...
        midterm_highest_in_class = max(midterm_scores)
        midterm_lowest_in_class = min(midterm_scores)
        ...
        result_stat = {
            ...
            'updated_by': request.user.profile.staff.surname.title() + ' ' + request.user.profile.staff.last_name.title(),
            'updated_by_id': request.user.profile.staff.id
    ...
    student_list = StudentsModel.objects.filter(student_class=student_class, class_section=class_section).order_by('surname')
...
def text_result_upload_view(request, student_pk):
    ...
    if request.method == 'POST':
        ...
        uploaded_result = ResultModel.objects.filter(session=session, term=term, student=student).first()
        if uploaded_result:
            uploaded_result.result = result_record
        else:
            uploaded_result = ResultModel.objects.create(session=session, term=term, student=student,
    ...
    allowed_subjects = TextResultCategoryModel.objects.filter(teachers__in=[staff])
...
def result_archive_check_view(request):
    ...
    if 'POST' == request.method:
        ...
        result_list = ResultModel.objects.filter(student_class=student_class, class_section=class_section).order_by(
            'student__surname')
        full_list = {}
...
class TextResultCategoryListView(LoginRequiredMixin, PermissionRequiredMixin, ListView):
    ...
    def get_queryset(self):
        school_setting = SchoolGeneralInfoModel.objects.first()
        if school_setting.separate_school_section:
            return TextResultCategoryModel.objects.filter(type=self.request.user.profile.type).order_by('name')
        else:
            return TextResultCategoryModel.objects.all().order_by('name')
