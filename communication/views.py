...
class SMSConfigurationListView(ListView):
    model = SMSConfigurationModel
    fields = '__all__'
    template_name = 'communication/sms_configuration/index.html'
    context_object_name = "sms_configuration_list"

    def get_queryset(self):
        school_setting = SchoolGeneralInfoModel.objects.first()
        if school_setting.separate_school_section:
            return SMSConfigurationModel.objects.filter(type=self.request.user.profile.type).order_by('name')
        else:
            return SMSConfigurationModel.objects.all().order_by('name')

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['form'] = SMSConfigurationForm

        return context


class SMSConfigurationUpdateView(SuccessMessageMixin, UpdateView):
    model = SMSConfigurationModel
    form_class = SMSConfigurationEditForm
...
class SMSConfigurationDeleteView(SuccessMessageMixin, DeleteView):
    model = SMSConfigurationModel
    success_message = 'SMS Configuration Deleted Successfully'
    fields = '__all__'
    template_name = 'communication/sms_configuration/delete.html'
    context_object_name = "sms_configuration"
...
class MessageCreateView(SuccessMessageMixin, CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        school_setting = SchoolGeneralInfoModel.objects.first()
        if school_setting.separate_school_section:
            context['message_list'] = SMSConfigurationModel.objects.filter(
                type=self.request.user.profile.type).order_by('created_at').reverse()
        else:
            context['message_list'] = SMSConfigurationModel.objects.all().order_by('created_at').reverse()
        return context
...
class MessageListView(ListView):
    ...
    def get_queryset(self):
        school_setting = SchoolGeneralInfoModel.objects.first()
        if school_setting.separate_school_section:
            return MessageModel.objects.filter(type=self.request.user.profile.type).order_by('created_at').reverse()
        else:
            return MessageModel.objects.all().order_by('created_at').reverse()
...
class MessageDetailView(DetailView):
    model = MessageModel
    fields = '__all__'
    template_name = 'communication/message/detail.html'
...
class MessageUpdateView(SuccessMessageMixin, UpdateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        school_setting = SchoolGeneralInfoModel.objects.first()
        if school_setting.separate_school_section:
            context['message_list'] = MessageModel.objects.filter(
                type=self.request.user.profile.type).order_by('created_at').reverse()
        else:
            context['message_list'] = MessageModel.objects.all().order_by('created_at').reverse()
...
def send_fee_reminder_auto_mail(request):
    smtp_connection, sender_email = check_mail_setup(request)
    if request.method == 'POST':
        email_list = []
        user_detail = {}

        debtor, no_mail, successful_mail, error_mail = 0, 0, 0, 0
        setting_info = SchoolGeneralInfoModel.objects.first()
        ...
        for student in student_list:
            if not student.email and not student.parent.email:
                no_mail += 1
                continue
            student_class = student.student_class
            class_section = student.class_section
            if setting_info.separate_school_section:
                if student_class and class_section:
                    termly_fee_list = FeeMasterModel.objects.filter(type=request.user.profile.type,
                                                                    fee__fee_occurrence='termly',
                                                                    student_class__in=[student_class.id],
                                                                    class_section__in=[class_section.id])
                    one_time_fee_list = FeeMasterModel.objects.filter(type=request.user.profile.type,
                                                                      student_class__in=[student_class.id],
            ...
            if fee_balance > 0:
                ...
                recipient_mail = []
                if student.email:
                    recipient_mail.append(student.email)
                if student.parent.email:
                    recipient_mail.append(student.parent.email)
                subject = 'Fee Payment Reminder'
                context = {'domain': get_current_site(request), 'subject': subject, 'school_info': setting_info,
                           'student': student, 'fee_balance': fee_balance, 'outstanding_fee': outstanding_fee,
                           'total_fee': outstanding_fee + fee_balance,
                           'fee_balance_in_word': num2words(fee_balance),
                           'outstanding_fee_in_word': num2words(outstanding_fee),
                           'total_in_word': num2words(fee_balance + outstanding_fee)}

                mail_subject = "{} - {}".format(setting_info.name.title(), subject.upper())

                html_message = render_to_string('communication/template/fee_reminder_auto_mail.html', context)
                plain_message = strip_tags(html_message)
                try:
                    mail_sent = send_mail(mail_subject, plain_message, sender_email, recipient_mail,
        ...
        messages.success(request,
            "Total Students: {}, Without Email: {}, With Email: {}, Total Debtors: {}, Successfully Sent: {}, Error Sending: {}".format(
                total_student, no_mail, total_student - no_mail, debtor, successful_mail, error_mail
            ))
        return redirect(reverse('user_account_auto_mail'))
    return render(request, 'communication/mail/auto_mail.html')
